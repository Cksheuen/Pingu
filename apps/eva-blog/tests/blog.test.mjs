...
import { findArticleBySlug, listPublishedArticles, matchesArticleQuery } from "../src/domain/publicBlog.js";
import { describeStatus } from "../src/domain/publicStatus.js";
...
test("public API requires an injected authenticated session for comments", () => {
  ...
  assert.throws(() => api.addComment(publishedArticle.id, "Before login"), /signed-in GitHub user/);
  const comment = api.addComment(publishedArticle.id, "Looks good.", {
    provider: "github",
    id: "github-reader",
    login: "eva-reader",
    name: "Eva Reader",
  ...
  assert.equal(typeof api.saveArticle, "undefined");
  assert.equal(typeof api.updateStatus, "undefined");
  assert.equal(typeof api.loginMock, "undefined");
});
