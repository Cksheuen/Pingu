...
export function createLocale({ browserLanguage = globalThis.navigator?.language, storage = getBrowserStorage() } = {}) {
...
export function describeLocalizedStatus(status, locale) {
  if (!status) return locale.t("status.empty");
  const kind = status.kind === "song"
    ? status.meta?.playing === false ? locale.t("status.paused") : locale.t("status.listening")
    : status.kind === "work"
  ...
  const title = status.kind === "song"
    ...
    : status.kind === "token"
      ? formatTokenUsage(status.meta, status.title, locale)
      : status.title;
  return `${kind}: ${title}${status.details ? ` — ${status.details}` : ""}`;
}

function formatTokenUsage(meta, fallback, locale) {
...
function writeStoredLocale(storage, value) {
  try {
