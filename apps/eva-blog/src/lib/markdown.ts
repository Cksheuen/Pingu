...
export function renderMarkdown(markdown: string): string {
  ...
  for (const line of lines) {
    ...
    if (line.startsWith("# ") || line.startsWith("## ") || line.startsWith("### ")) { if (listOpen) { html.push("</ul>"); listOpen = false; } const depth = line.match(/^#+/)?.[0]?.length ?? 1; const text = line.slice(depth + 1); html.push(`<h${Math.min(3, depth)} id="${escapeAttr(slugify(text))}">${escapeHtml(text)}</h${Math.min(3, depth)}>`); }
...
export function outlineFromContent(content: string): OutlineItem[] {
