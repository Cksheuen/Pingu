...
import { renderMarkdown, outlineFromContent } from "../lib/markdown";
import { Comments } from "../components/Comments";
import { EmptyState } from "../components/EmptyState";
import { ReadingProgress } from "../components/ReadingProgress";
import { BackToTop } from "../components/BackToTop";
import type { ArticleReaderPayload, ArticleOutlineItem, PublicArticle, Comment } from "../types";
...
export function ArticleView({ article, articles, loading, comments, commentsLoading, articleRef, progressRef, backToTopRef, onBackToTop, onCommentSubmit, onCopyLink }: ArticleViewProps) {
  ...
  const outline = (article as ArticleReaderPayload).outline || outlineFromContent(article.content);
  const summary = article.summary?.text || t("reader.summaryPending");
  const related = (article as ArticleReaderPayload).related || articles.filter((candidate) => candidate.id !== article.id && (candidate.tags || []).some((tag) => article.tags?.includes(tag))).slice(0, 3);
          ...
          <aside className="reader-aside">
            {outline.length > 0 && (
              <div className="toc">
                ...
                {outline.map((item: ArticleOutlineItem) => (
          ...
          <div className="reader-manuscript">
            ...
            {related.length > 0 && (
              <section className="related-section">
                ...
                <div>
                  {related.map((item: PublicArticle) => (
