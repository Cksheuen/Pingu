...
const server = createServer(async (request: IncomingMessage, response: ServerResponse) => {
  try {
    ...
    response.writeHead(httpError.status || 500, { "Content-Type": "text/plain" });
    response.end(httpError.status === 404 ? "Not found" : String(httpError.message || error));
  }
});
...
async function proxyApi(request: IncomingMessage, response: ServerResponse, url: URL): Promise<void> {
  ...
  headers.delete("host");
  const apiResponse = await apiWorker.fetch(new Request(`http://${host}:${port}${url.pathname}${url.search}`, { method: request.method, headers, body: body ? new Uint8Array(body) : undefined }));
...
async function resolvePath(pathname: string): Promise<string> {
