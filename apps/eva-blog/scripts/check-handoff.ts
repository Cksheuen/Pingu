...
for (const file of requiredFiles) {
  try {
    ...
    failures.push(`${file}: missing or unreadable (${(error as Error).message})`);
  }
}
...
if (!failures.length) {
  ...
  let vercel: { buildCommand?: string; outputDirectory?: string } | undefined;

  try {
    vercel = JSON.parse(contents.get("vercel.json") as string);
  } catch (error) {
    failures.push(`vercel.json: invalid JSON (${(error as Error).message})`);
  }

  let packageJson: { scripts?: Record<string, string> } | undefined;
  try {
    packageJson = JSON.parse(packageJsonText as string);
  } catch (error) {
    failures.push(`package.json: invalid JSON (${(error as Error).message})`);
  }
...
function requireIncludes(content: string | undefined, needle: string, message: string): void {
  if (!content?.includes(needle)) {
    failures.push(message);
  }
}

function requireMatches(content: string | undefined, pattern: RegExp, message: string): void {
