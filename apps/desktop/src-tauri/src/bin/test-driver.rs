...
fn invoke(command: &str, args: &Value) -> Result<Value, String> {
    ...
    let result = match command {
        ...
        "get_gate_settings" => json!(pingu_lib::gate::get_settings()?),
        "configure_gate" => {
            let access_link = get_string_arg(args, "accessLink")?;
            json!(pingu_lib::gate::configure(&access_link)?)
        }
        "set_gate_enabled" => {
            let enabled = args
                .get("enabled")
                .and_then(|value| value.as_bool())
                .ok_or_else(|| "Missing bool arg: enabled".to_string())?;
            json!(pingu_lib::gate::set_enabled(enabled)?)
        }
        "renew_gate_lease" => json!(pingu_lib::gate::renew()?),
