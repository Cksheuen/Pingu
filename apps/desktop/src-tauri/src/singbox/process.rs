...
impl SingBoxProcess {
    pub fn new() -> Self {
        Self::with_binary(crate::resolve_sing_box_path())
    }
    ...
    pub fn stop(&self) -> Result<(), String> {
        ...
        match (kill_error, wait_error) {
            (None, None) => Ok(()),
            (Some(kill), None) => Err(format!("Failed to kill sing-box: {}", kill)),
            (None, Some(wait)) => Err(format!("Failed to wait on sing-box: {}", wait)),
            (Some(kill), Some(wait)) => Err(format!(
                "Failed to kill sing-box: {}; failed to wait on sing-box: {}",
                kill, wait
            )),
        }
    }
