...
test("MC-01 前端 API 调用链可以走完整个稳定性闭环", { concurrency: false }, async () => {
...
test("MC-02 前端 API 调用链可以覆盖失败后修正恢复", { concurrency: false }, async () => {
  await withRustInvokeHarness(async () => {
    resetConnectionStore();
...
test("MC-03 前端 API 调用链可以覆盖增量配置、回收与回退", { concurrency: false }, async () => {
  await withRustInvokeHarness(async () => {
    resetConnectionStore();
...
test("MC-04 前端 API 调用链可以覆盖多节点多规则组排列组合与在线回退", { concurrency: false }, async () => {
  await withRustInvokeHarness(async () => {
    resetConnectionStore();
