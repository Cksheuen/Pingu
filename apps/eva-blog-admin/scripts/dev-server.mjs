...
import { createFileStorage } from "../../eva-blog/server/devStorage.js";

const port = Number.parseInt(process.env.PORT || argValue("--port") || "4174", 10);
const host = process.env.HOST || argValue("--host") || "127.0.0.1";
const root = process.cwd();
const seedData = JSON.parse(await readFile(join(root, "data/seed.json"), "utf8"));
const statePath = process.env.EVA_BLOG_LOCAL_STATE || join(root, "../../.local/eva-blog-state.json");
const apiWorker = createWorker({
  seedData,
  storage: createFileStorage(statePath, seedData),
  env: {
    ...process.env,
    ALLOWED_ORIGINS: process.env.ALLOWED_ORIGINS || `http://${host}:4175`,
    ALLOWED_REDIRECT_ORIGINS: process.env.ALLOWED_REDIRECT_ORIGINS || `http://${host}:4175`
  }
});
...
server.listen(port, host, () => {
  console.log(`Eva Author Workspace dev server: http://${host}:${port}`);
});
...
async function resolvePath(pathname) {
  ...
  try {
    ...
    if (!extname(pathname)) {
      await readFile(join(root, "app/index.html"));
      return join(root, "app/index.html");
    }
